...
/**
 * _is_realloc - Reallocates a memory block
 ...
 */
void *_is_realloc(void *ptr, unsigned int old_size, unsigned int new_size)
{
	void *new_ptr;
...
int *heap_to_sorted_array(heap_t *heap, size_t *size)
{
	...
	if (heap != NULL)
	{
		...
		while (root != NULL)
		{
			...
			array = _is_realloc(array, sizeof(int) * n, sizeof(int) * (n + 1));
			*(array + n) = val;
			n++;
		}
	}
